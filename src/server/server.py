...
def add_patient():
    ...
    cursor.execute(insert_query, (data['patientname'], data['mobile'],data['visitedfrom'], data['age'],data['blood_pressure'], data['dateofvisit'], data['gender'], data['department'], data['doctors_names']))
    print("patient added!")
    conn.commit()
    cursor.close()
    conn.close()
